...
for train_site in ['nbn_duck']:
    for add_site in ['nbn', 'duck']:
        for percentage in [0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95]:
            for runno in range(10):
                lr = 0.01

                ##saveout info
                model_name = 'resnet512_add{}_percentage_{}_{}'.format(add_site, percentage,runno)
                ...
                for tt in train_site_list:
                    ...
                    if tt == add_site:
                        cutlen = int(len(valfile) * percentage)
                        random.shuffle(valfile)
                        valfiles += valfile[:cutlen]

                        cutlen = int(len(trainfile)*percentage)
                        random.shuffle(trainfile)
                        trainfiles += trainfile[:cutlen]
