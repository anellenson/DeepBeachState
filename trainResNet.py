...
CNNtype = 'resnet'
augtype = 'noise.vcut.streaks'
train_site = 'nbn'
lr = 0.01

##saveout info
for augtype in ['vcut', 'streaks', 'noise', 'noise.vcut.streaks', 'spz.vcut.streaks']:
    for run in range(10):
        model_name = 'resnet_{}_{}'.format(augtype, run)
        ...
        def confusion_results(test_site, augtype):
            with open('labels/{}_daytimex_valfiles.no_aug.pickle'.format(test_site)) as f:
        ...
        for test_site in ['duck', 'nbn']:
            CNN_site_preds, truth_site = confusion_results(test_site, augtype)
