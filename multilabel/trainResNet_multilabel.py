...
plot_dir  = '/home/server/pi/homes/aellenso/Research/DeepBeach/plots/resnet/multilabel/'
no_epochs = 30
step_size = 12 #when to decay the learning rate
#plot_fname_percent = plot_dir + 'resnetTrainInfo.baseline' + str(no) + '.' + type + '.x{0:3d}.y{1:3d}.lr{2:3.2e}.ss{3:2d}.bs{4:2d}.mo{5:3.2e}.gamma{6:3.2e}.png'.format(res1,res2,lr,step_size,batch_size,momentum,gamma)
#plot_fname_count = plot_dir + 'resnetTrainInfo.baseline' + str(no) + '.' + type  + '.x{0:3d}.y{1:3d}.lr{2:3.2e}.ss{3:2d}.bs{4:2d}.mo{5:3.2e}.gamma{6:3.2e}.png'.format(res1,res2,lr,step_size,batch_size,momentum,gamma)
#grouped_confname = plot_dir + 'resnetGroupedMatrix.baseline' + str(no) + '.' + type + '.x{0:3d}.y{1:3d}.lr{2:3.2e}.ss{3:2d}.bs{4:2d}.mo{5:3.2e}.gamma{6:3.2e}.png'.format(res1,res2,lr,step_size,batch_size,momentum,gamma)
#plot_title = 'baseline' + str(no) + '.' + type + str(res1) + '.' + str(res2) + '.lr' + str(lr) + '.ss' + str(step_size) + '.bs' + str(batch_size) + 'mo' + str(momentum) + 'gamma' + str(gamma)
...
labels_df = pd.read_pickle('labels/strlabel_df.pickle')
multilabel_dict = {}
...
#This loads the labels dictionary (for the ArgusDataset) and the labels dataframe (to create partitions) from the matfiles
pids = list(multilabel_dict.keys())
labels = list(multilabel_dict.values())
...
train_ds = ArgusDS.ArgusTrainDS(basedir, partition['train'], multilabel_dict, transform = train_transform)
train_dl = torch.utils.data.DataLoader(train_ds, batch_size=batch_size, shuffle = 4) #change batch size?

val_ds = ArgusDS.ArgusTrainDS(basedir, partition['val'], multilabel_dict, transform = test_transform)
...
torch.save(model_conv.state_dict(), 'models/multilabel/resnet50.pth')
